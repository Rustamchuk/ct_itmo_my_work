...
public class Context
{
    ...
    public int T4 { get; private set; }
    public int T5 { get; private set; }
    public int T6 { get; private set; }
    public List<List<long>> GigabaitDataScience;
    public new List<Edge> SomeCleverReallyNameEdLi;
    ...
    public void Init()
    {
        ...
        GigabaitDataScience = new List<List<long>>(new List<long>[N]);
        for (var i = 0; i < N; i++)
        {
            GigabaitDataScience[i] = new List<long>();
        }
        SomeCleverReallyNameEdLi = new List<Edge>();
    }

    public EdgeContext InitEdge()
    {
        var input = Console.ReadLine().Split();
        var u =  Convert.ToInt32(input[0]) - 1;
        var v =  Convert.ToInt32(input[1]) - 1;
        var c = long.Parse(input[2]);
        var cost = long.Parse(input[3]);
        var ctx = new EdgeContext(u, v, c, cost);
        return ctx;
    }
    ...
    {
        long ans = 0;
        for (var i = 0; i < SomeCleverReallyNameEdLi.Count; i += 2)
        {
            ans += SomeCleverReallyNameEdLi[i].F * SomeCleverReallyNameEdLi[i].Cost;
        }
...
public class EdgeContext
{
    ...
    public EdgeContext(int u, int v, long c, long cost)
    {
        U = u;
        V = v;
        C = c;
...
public class Program
{
    public static void Main()
    {
        ...
        for (var i = 0; i < ctx.M; i++)
        {
            ...
            {
                case 1:
                    ctx.SomeCleverReallyNameEdLi.Count();
                    break;
                case 2:
                    ctx.SomeCleverReallyNameEdLi.Count();
                    break;
        ...
        MinCostFlow(ctx.GigabaitDataScience, ctx.SomeCleverReallyNameEdLi, 0, ctx.N - 1);
        ctx.PrintResult();
    }

    private static void AddEdge(Context ctx, EdgeContext input)
    {
        var e = new Edge(input.C, 0, input.V, ctx.SomeCleverReallyNameEdLi.Count, input.Cost)
        {
            RevNum = ctx.SomeCleverReallyNameEdLi.Count + 1,
            From = input.U
        };
        ctx.SomeCleverReallyNameEdLi.Add(e);
        ctx.GigabaitDataScience[input.U].Add(e.Num);

        var e1 = new Edge(0, 0, input.U, ctx.SomeCleverReallyNameEdLi.Count, -input.Cost)
        {
            RevNum = e.Num,
            From = input.V
        };
        ctx.SomeCleverReallyNameEdLi.Add(e1);
        ctx.GigabaitDataScience[input.V].Add(e1.Num);
    }

    public static (long, bool, List<long>) Dijkstra(List<List<long>> g, long s, long t, List<Edge> edgesList)
    {
        var q = new SortedSet<(long, long)>();
        var d = Enumerable.Repeat(Context.Inf, g.Count).ToList();
        var parent = new List<(long, long)>(new (long, long)[g.Count]);
        ...
        while (q.Count > 0)
        {
            ...
            foreach (var it in g[(int)v])
            {
                var e = edgesList[(int)it];
                var to = e.To;
                var len = e.Cost;

                if (d[(int)v] + len >= d[(int)to] || e.F >= e.C) continue;
                q.Remove((d[(int)to], to));
                d[(int)to] = d[(int)v] + len;
                q.Add((d[(int)to], to));
                parent[(int)to] = (v, it);
            }
        }

        var hasPath = (d[(int)t] != Context.Inf);
        if (!hasPath)
        {
            return (0, hasPath, new List<long>());
        }

        var edgesPath = new List<long>();
        var vPair = parent[(int)t];
        while (vPair.Item1 != -1)
        {
            edgesPath.Add(vPair.Item2);
            vPair = parent[(int)vPair.Item1];
        }
        edgesPath.Reverse();
        var minCapacity = edgesPath.Select(it => edgesList[(int)it].C - edgesList[(int)it].F).Prepend(Context.Inf).Min();
        return (minCapacity, hasPath, edgesPath);
    }

    public static void MinCostFlow(List<List<long>> g, List<Edge> edgesList, long s, long t)
    {
        while (true)
        {
            var (minCapacity, hasPath, edgesInPath) = Dijkstra(g, s, t, edgesList);
            if (!hasPath)
            {
                break;
            }

            foreach (var eInd in edgesInPath)
            {
                var e = edgesList[(int)eInd];

                edgesList[(int)eInd].F += minCapacity;
                edgesList[(int)e.RevNum].F -= minCapacity;
            }
        }
    }
}
...
public class Edge
{
    ...
    public Edge(long c, long f, long to, long num, long cost)
    {
        C = c;
        F = f;
        To = to;
