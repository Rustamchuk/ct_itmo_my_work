...
// //         {
    ...
    private void ДобавитьТочку(Программа.Точка точка)
    {
        Точки.Add(точка);
    }
    ...
    {
        Console.WriteLine(ФорматироватьРезультат(результат));
    }

    private string ФорматироватьРезультат(double результат)
    {
        return результат.ToString("F6", CultureInfo.InvariantCulture);
    }
}

public class Программа
{
    public struct Точка : IComparable<Точка>
    {
        public long КоординатаX, КоординатаY;

        public Точка(long x, long y)
        {
            КоординатаX = x;
            КоординатаY = y;
        }

        public int CompareTo(Точка другаяТочка)
        {
            return КоординатаX != другаяТочка.КоординатаX ? КоординатаX.CompareTo(другаяТочка.КоординатаX) : КоординатаY.CompareTo(другаяТочка.КоординатаY);
        }
    }

    public static double Периметр(List<Точка> точки)
    {
        if (точки.Count <= 1)
            return 0;

        точки.Sort();
        var оболочка = ПостроитьОболочку(точки);
        return ВычислитьПериметр(оболочка);
    }

    private static List<Точка> ПостроитьОболочку(List<Точка> точки)
    {
        var оболочка = new List<Точка>();
        ПостроитьНижнююОболочку(точки, оболочка);
        ПостроитьВерхнююОболочку(точки, оболочка);
        УдалитьПоследнююТочку(оболочка);
        return оболочка;
    }

    private static void ПостроитьНижнююОболочку(List<Точка> точки, List<Точка> оболочка)
    {
        foreach (var точка in точки)
        {
            ДобавитьТочкуВНижнююОболочку(оболочка, точка);
        }
    }

    private static void ДобавитьТочкуВНижнююОболочку(List<Точка> оболочка, Точка точка)
    {
        while (СледуетУдалитьПоследнююТочкуИзНижнейОболочки(оболочка, точка))
        {
            УдалитьПоследнююТочку(оболочка);
        }
        оболочка.Add(точка);
    ...
    private static bool СледуетУдалитьПоследнююТочкуИзНижнейОболочки(List<Точка> оболочка, Точка точка)
    {
        return оболочка.Count >= 2 && ВекторноеПроизведение(оболочка[оболочка.Count - 2], оболочка[оболочка.Count - 1], точка) <= 0;
    }

    private static void ПостроитьВерхнююОболочку(List<Точка> точки, List<Точка> оболочка)
    {
        var t = оболочка.Count + 1;
        for (var i = точки.Count - 2; i >= 0; i--)
        {
            var точка = точки[i];
            ДобавитьТочкуВВерхнююОболочку(оболочка, точка, t);
        }
    }

    private static void ДобавитьТочкуВВерхнююОболочку(List<Точка> оболочка, Точка точка, int t)
    {
        while (СледуетУдалитьПоследнююТочкуИзВерхнейОболочки(оболочка, точка, t))
        {
            УдалитьПоследнююТочку(оболочка);
        }
        оболочка.Add(точка);
    ...
    private static bool СледуетУдалитьПоследнююТочкуИзВерхнейОболочки(List<Точка> оболочка, Точка точка, int t)
    {
        return оболочка.Count >= t && ВекторноеПроизведение(оболочка[оболочка.Count - 2], оболочка[оболочка.Count - 1], точка) <= 0;
    }

    private static void УдалитьПоследнююТочку(List<Точка> оболочка)
    {
        оболочка.RemoveAt(оболочка.Count - 1);
    }

    private static double ВычислитьПериметр(List<Точка> оболочка)
    {
        double периметр = 0;
        for (var i = 0; i < оболочка.Count; i++)
        {
            var точка1 = оболочка[i];
            var точка2 = оболочка[(i + 1) % оболочка.Count];
            периметр += ВычислитьРасстояние(точка1, точка2);
        }
        return периметр;
    }

    private static double ВычислитьРасстояние(Точка точка1, Точка точка2)
    {
        return Math.Sqrt(ВычислитьКвадратРасстояния(точка1, точка2));
    }

    private static double ВычислитьКвадратРасстояния(Точка точка1, Точка точка2)
    {
        return (точка1.КоординатаX - точка2.КоординатаX) * (точка1.КоординатаX - точка2.КоординатаX) + (точка1.КоординатаY - точка2.КоординатаY) * (точка1.КоординатаY - точка2.КоординатаY);
    }

    private static long ВекторноеПроизведение(Точка точкаO, Точка точкаA, Точка точкаB)
    {
        return ВычислитьВекторноеПроизведение(точкаA, точкаB, точкаO);
    }

    private static long ВычислитьВекторноеПроизведение(Точка точкаA, Точка точкаB, Точка точкаO)
    {
        return (точкаA.КоординатаX - точкаO.КоординатаX) * (точкаB.КоординатаY - точкаO.КоординатаY) - (точкаA.КоординатаY - точкаO.КоординатаY) * (точкаB.КоординатаX - точкаO.КоординатаX);
    }

    public static void Main()
    {
        var контекст = new Контекст();
        контекст.ПрочитатьТочки();
        var результат = Периметр(контекст.Точки);
        контекст.ВывестиРезультат(результат);
    }
}
