...
//    <td>{level}</td>
    ...
    private final Map<String, App> apps = new TreeMap<>(Map.ofEntries(
            app("Client 1", "java", "-cp", CLASSPATH, "info.kgeorgiy.ja.nazarov.bank.Client", "client-1"),
            app("Client 2", "java", "-cp", CLASSPATH, "info.kgeorgiy.ja.nazarov.bank.Client", "client-2"),
            app("Client 3", "java", "-cp", CLASSPATH, "info.kgeorgiy.ja.nazarov.bank.Client", "client-3"),
            app("Server 1", "java", "-cp", CLASSPATH, "info.kgeorgiy.ja.nazarov.bank.Server", "8881"),
            app("Server 2", "java", "-cp", CLASSPATH, "info.kgeorgiy.ja.nazarov.bank.Server", "8882"),
    ...
    private String getLog() {
        synchronized (logs) {
            return logs.stream().map(LogEntry::html).collect(Collectors.joining());
        }
    }
    ...
    private String buttons(final String action) {
        return template(BUTTONS_TEMPLATE, Map.of(
                "action", action,
                "buttons", apps.keySet().stream().map(name -> template(BUTTON_TEMPLATE, Map.of(
                        "action", action,
                        "name", name
    ...
    private static class LogEntry {
        ...
        public LogEntry(final String app, final LogLevel level, final String message) {
            html = template(LOG_ENTRY_TEMPLATE, Map.of(
